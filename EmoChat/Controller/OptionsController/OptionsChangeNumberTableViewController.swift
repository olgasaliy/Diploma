...
class OptionsChangeNumberTableViewController: UITableViewController, UITextFieldDelegate, RegexCheckProtocol {
    ...
    @IBAction func phoneNumberEdited(_ sender: UITextField) {
        if phoneIsValid(uname: sender.text) {
            infoLabel.text = NSLocalizedString("Phone number is valid", comment: "Phone number is valid")
            infoLabel.textColor = UIColor.darkGray
        } else {
            infoLabel.printError(errorText: "Enter valid phone number")
